...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        for point in self.points:
            for otherpoint in other.points:
                ...
                if shortest==None or shortest>dist:
                    shortest=dist
    ...
    def maxLinkageDist(self, other):
        ...
        for point in self.points:
            for otherpoint in other.points:
                dist=point.distance(otherpoint)
                if longest==None or longest<dist:
                    longest=dist
        return longest
    ...
    def averageLinkageDist(self, other):
        ...
        return sumdist/float(len(self.points)*len(other.points))
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        self.add(Cluster(
            list(set([c for c in c1.points]) | set([c for c in c2.points])),
            Point
        ))
    ...
    def findClosest(self, linkage):
        ...
        bestDist = None
        pair = (self.members[0], self.members[0])
        for c1 in self.members:
            for c2 in self.members:
                if c1 == c2:
                    continue
                dist = linkage(c1,c2)
                if dist < bestDist or bestDist==None:
                    bestDist = dist
                    pair = (c1, c2)
        return pair
